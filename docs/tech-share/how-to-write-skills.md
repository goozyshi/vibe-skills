...
description: >-                   #    AI 根据描述匹配激活
  PDF 文档处理与转换：提取文本/图片/表格，合并、拆分、旋转页面。
  当用户需要处理 PDF 文件时触发。
...
这个比较反直觉——人的直觉是"越详细说明要做什么，结果越好"，但实践中恰好相反：当你描述"做什么"时，AI 面对的是无限大的解空间，它需要在无数种可行路径中随机选择；而当你声明"不做什么"时，你是在帮 AI 剪枝，把搜索范围收窄到期望区间。就像给 AI 一张地图时，标记出边界线比描述中心区域更有效。

**可行域的核心问题：**

| 约束方式 | 效果 | 结果 |
|----------|------|------|
| 描述"做什么" | 定义无限大的解空间 | AI 随机游走，结果不可预测 |
| 声明"不做什么" | 帮 AI 剪枝 | AI 行为空间被收窄到期望范围 |
...
name: pdf-processor
description: |
  PDF 文档处理与转换：提取文本/图片/表格，合并、拆分、旋转页面，添加水印或数字签名。
  当用户需要处理 PDF 文件时触发。
---
```
...
回答以下问题来撰写描述：
1. **功能边界**：这个 Skill 支持哪些具体功能？（构思 2-3 个核心场景，写入时用并列短句串成一两句话，勿用 (1)(2)(3) 编号）
2. **触发词汇**：用户会用什么词汇来描述这些需求？（记录典型表达）
3. **边界说明**：哪些相似但不支持的功能需要明确排除？

```yaml
description: |
  [核心能力一两句话；多项并列用顿号、逗号或分句衔接]。
  当用户 [描述触发条件] 时触发。
  [可选：一行排除易混淆场景]。
```
...
description: |
  从 PDF 提取纯文本：单页或全文，保留基本排版结构。
  当用户需要读取或分析 PDF 文本内容时触发。
