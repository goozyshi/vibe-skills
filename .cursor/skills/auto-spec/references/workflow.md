...
**Contract test**：先识别产品用户、开发者、Agent 或 CI 等消费者。任一消费者获得、失去或观察到不同能力 → L2；能证明全部消费者不受影响 → L0；无法证明 → L1。生成物继承造成它变化的父变更等级，不独立降级。spec 已覆盖 → 对照 spec；spec 为空只表示没有基线。

```
L0（contract unchanged）：修现有实现、私有重构、补测试或清理无消费者残留。

L1（contract unknown）：只读调查消费者、active change 与 spec。

L2（contract changed）：建立或续用 change；spec 未覆盖时同时建立基线。
```
